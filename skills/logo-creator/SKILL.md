...
→ REJECT — re-run with local fallback: python3 <skill-dir>/scripts/remove_bg.py input.webp output.png
```
...
python3 <skill-dir>/scripts/remove_bg.py \
...
5. **Build contact sheet, open it, and give the user the path:**
   ```bash
   python3 <skill-dir>/scripts/create_logo_sheet.py \
...
```bash
python3 <skill-dir>/scripts/compose_logo.py \
  --brand "BrandName" \
