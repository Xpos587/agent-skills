...
2. **Run the validator** (optional but recommended):
   ```bash
   python <skill-dir>/scripts/validate-drawio.py <path-to-file.drawio>
   ```
...
```bash
python <skill-dir>/scripts/add-shape.py docs/arch.drawio "New Service" 700 380
```
...
```bash
python <skill-dir>/scripts/validate-drawio.py <file.drawio>
```
