...
For D1, R2, KV, Workers, Vectorize, and Queues operations, see the verified reference:

**[references/developer-platform-api.md](references/developer-platform-api.md)**

Contains tested patterns for:
- D1 queries, raw SQL, cross-database operations
- R2 bucket management and S3-compatible operations
- KV bulk read/write (up to 10,000 keys)
- Worker bindings and settings
- Vectorize queries and deletions
- Queue message injection
- Fleet-wide audit scripts

All examples in the reference file are tested against real Cloudflare APIs.
