...
async def record_screen(func, video_dir: Optional[str] = None, fps: int = 10, max_frames: int = 300, max_duration: int = 60):
    ...
    Args:
        ...
        max_frames: Maximum frames to record (default: 300 = 30s @ 10fps)
        max_duration: Maximum recording duration in seconds (default: 60)

    Returns:
        {video_path, result, frames}
    ...
    if screenshots:
        if not _VIDEO_LIBS_AVAILABLE:
            # No video libs available, keep all screenshots as PNG sequence
            frames_dir = screenshot_dir
            fallback_base = str(Path(video_dir) / f"recording-{timestamp}")
            
            # Create index file for easy viewing
            index_path = Path(fallback_base + "_index.txt")
            index_path.write_text("\n".join(s["path"] for s in screenshots))
            
            fallback_path = f"{fallback_base}_frames ({len(screenshots)} frames in {frames_dir})"
        ...
        try:
            ...
            # Save as MP4 (v3 API) with timeout
            def write_video():
                iio.imwrite(video_path, video_array, fps=fps, codec='libx264')
            
            await asyncio.wait_for(asyncio.to_thread(write_video), timeout=30.0)
            ...
            # Fallback: keep all screenshots as PNG sequence
            frames_dir = screenshot_dir
            fallback_base = str(Path(video_dir) / f"recording-{timestamp}")
            ...
            video_path = f"{fallback_base}_frames ({len(screenshots)} frames in {frames_dir})"
