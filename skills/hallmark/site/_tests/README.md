...
The current generation reflects **v0.6.0** of the skill (the skill itself is now **v1.0.0** — see `SKILL.md` frontmatter). Pre-flight scan, project-memory rotation, and the upfront preview block all fire. Each `brief.md` walks through Steps 0 → 6 of the design flow so the discipline is visible.
