# Design flow (default verb)

Loaded when the user invokes Hallmark without a verb prefix (default build).
...
Before loading any visual ruleset, **read the slim index at [`macrostructures.md`](macrostructures.md) and pick one of the twenty-one named macrostructures.** The index is one-line-per-macro; pick a name, then **load ONLY that one per-macro file** from `macrostructures/` (e.g. `macrostructures/05-workbench.md`). Do not load the whole catalogue — that's ~37 KB of dead weight for a single pick. Each macrostructure is a complete page-shape — heading placement, body composition, divider language, button voice, image treatment, reveal — bundled as a single named choice. Picking one named macrostructure is faster and more varied than choosing six independent axes from scratch.
...
The per-theme axis values live in [`theme-axes.md`](theme-axes.md) — a machine-readable table of 22 themes × 3 axes. Load it instead of `tokens.css` when comparing theme distances.
...
The macrostructure picks five of the six structural axes for you; you only need to pick the reveal yourself. The deeper axis catalogue is still in [`structure.md`](structure.md) when you need to deviate from the macrostructure's defaults.

**Pick a nav archetype (N1–N10) and a footer archetype (Ft1–Ft8) at this step.** They are not optional chrome; they are part of the page's structural fingerprint. Read the slim index at [`component-cookbook.md`](component-cookbook.md) and the routing tables at its bottom — the genre's default plus the acceptable alternates. Then **load ONLY the picked archetype files** from `components/` (e.g. `components/n5-floating-pill.md` + `components/ft5-statement.md`). A typical build loads 5–7 archetype files total (1 hero + 1 section head + 1–2 features + 1 CTA + 1 footer + 1 nav). Do not load the cookbook end-to-end — that's ~55 KB of archetypes you won't use. State both picks alongside the macrostructure: *"Macrostructure: Marquee Hero. Nav: N5 Floating pill. Footer: Ft5 Statement. Theme: Bloom."*
...
**Multi-page projects without `design.md`.** When building a second or third page for a project that lacks `design.md`, offer once: *"This project now has multiple Hallmark pages. Want me to lock the system into a `design.md` so all pages share a consistent design language? Say `lock the system`."* If the user declines, diversification rules still apply — each page picks a different macrostructure, but the user opted out of system-level consistency.
...
## Step 2.6 · Theme route — studied-DNA, catalog, or custom
...
A custom theme is a **complete** OKLCH palette + font pairing tuned to the brief — not a one-off colour swap, not an excuse to bypass the rules. Every constraint in [`color.md`](color.md), [`typography.md`](typography.md), and [`anti-patterns.md`](anti-patterns.md) still applies. The slop-test gates fire unchanged. The Step 5 preview block surfaces the palette + pairing in plain text **before** any code is emitted, so the user can redirect.
...
The non-negotiables live in this `references/` directory. **Be precise about what to load when. Discipline matters — over-eager loading is the largest avoidable cost of running Hallmark.**
...
- [`macrostructures.md`](macrostructures.md) — slim index of the 21 macros. Pick one name from the index, then load ONLY `macrostructures/<NN-slug>.md` for that pick. **Never load the whole index plus more than one per-macro file in a single build.** ~30 lines per per-macro file vs. 660 lines for the old monolith.
- [`component-cookbook.md`](component-cookbook.md) — slim index of 46 component archetypes (9 heroes, 5 section heads, 6 features, 4 CTAs, 4 testimonials, 8 footers, 10 navs) + the nav + footer routing tables at the bottom. Pick your archetype codes (H#, S#, F#, C#, T#, Ft#, N#) from the index, then load ONLY the matching `components/<code>-<slug>.md` files. A typical build loads 5–7 archetype files. **Loading the cookbook end-to-end or pre-loading more than one archetype per category is the single biggest token waste in the skill — don't.**
- [`theme-axes.md`](theme-axes.md) — 22 themes × 3 diversification axes. Load when comparing theme distances for the diversification rule.
...
- [`slop-test.md`](slop-test.md) — **strictly Step 7, after Build.** The 69 gates are a post-emit check, not a pre-emit reference. Pre-loading slop-test.md costs ~7K tokens for nothing — the gates inform fixes, not generation. If a gate fails at Step 7, fix and re-test; do not consult the file earlier "to know what to avoid" — that's what `anti-patterns.md` is for.
...
- **Slop test** · pending (run after Build)
...
6. **Slop test** — `pending (run after Build)` at Preview time. After Step 7 completes, update to `69 / 69 ✓` if all gates pass, or `N / 69 — fails: <gate numbers>` if any are open.
...
Four worked sample preview blocks (Long Document, Bento Grid, Manifesto, Custom) live in [`preview-examples.md`](preview-examples.md) — load that file only if the bullet-list spec above isn't scaffolding enough on its own. Most builds don't need it.
...
Before handing back, run the output through the 69-gate slop test in [`slop-test.md`](slop-test.md). Every answer must be **no**. Load that file at this step (not earlier — it isn't needed until handoff). The active genre matters: some gates are universal, some are genre-scoped (atmospheric loosens the radial-bloom gate; modern-minimal loosens the zero-chroma neutral gate; etc.). The full per-genre overrides are listed inline in `slop-test.md`.

After running the slop test, update the Step 5 preview block's Slop test row with actual results. The preview is the durable summary; it's wrong to ship if it lies.
...
Load [`contract.md`](contract.md) once, at handoff time, for the full output contract and scope-of-skill rules.
